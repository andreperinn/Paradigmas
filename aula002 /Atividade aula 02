...
Custo de programação: mesmo que o código gerado fosse um pouco menos eficiente, o tempo economizado em escrever, depurar e manter programas em uma linguagem de alto nível compensava largamente, pois o custo humano em horas de progamação começava a pesar mais que o custo de máquina.
...
Pascal, criada por Niklaus Wirth, herdou de ALGOL a estrutura em blocos e a ênfase em programação estruturada, mas simplificou a linguagem, tornando-a mais voltada ao ensino e à clareza.
C, criada por Dennis Ritchie, também herdou conceitos estruturais de ALGOL (via B/BCPL), mas priorizou eficiência, controle de baixo nível sobre memória e proximidade com hardware — características que tornaram C a base de sistemas operacionais como Unix.
...
Ada foi encomendada pelo Departamento de Defesa dos EUA para unificar centenas de linguagens usadas em sistemas embarcados militares, priorizando confiabilidade em ambientes onde falhas podem ser catastróficas (armamentos, aviônica, controle de mísseis).
...
Java: foi projetada para ser orientada a objetos de forma mais disciplinada que C++ (sem herança múltipla direta, sem ponteiros explícitos), e adotou uma estratégia de portabilidade baseada na JVM (Java Virtual Machine) — o código é compilado para bytecode intermediário, que roda em qualquer plataforma com uma JVM instalada, seguindo o lema "escreva uma vez, rode em qualquer lugar" ("write once, run anywhere").
